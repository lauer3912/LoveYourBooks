...
这里`&`符号是一个难点，我们不妨先不要关注这个符号，简单的把它看成是`Slice`的甲鱼臀部——规定。另外，同样的，`Slice`也是可以通过下标的方式访问其元素，下标也是从0开始的哟。
你可以这么声明并使用一个`Slice`：
...
你可以用`str`来声明一个字符串，事实上，Rust中，所有用`""`双引号包裹起来的都可以称为`&str`(注意这个`&`,这是难点，先不用管他。)，但是这个类型被单独用的情况很少，因此，我们将在下一节着重介绍字符串类型。
